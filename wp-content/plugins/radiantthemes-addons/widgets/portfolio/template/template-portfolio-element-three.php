...
$output = '<div class="rt-portfolio-box element-three row isotope ' . esc_attr( $settings['radiant_portfolio_extra_class'] ) . '" id="' . esc_attr( $portfolio_id ) . '">';
// WP_Query arguments.
global $wp_query;
$args     = array(
	'post_type'      => 'portfolio',
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		if ( $settings['radiant_portfolio_add_zoom'] ) {
			$output .= '<a class="portfolio-zoom fancybox" data-fancybox="gallery-' . get_queried_object_id() . '" href="' . get_the_post_thumbnail_url( get_the_ID(), 'full' ) . '"><i class="fa fa-search"></i></a>';
		}
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</div>';
