...
<?php
	$data_nicescroll_cursorcolor = ( ! empty( ryse_global_var( 'scrollbar_color', 'color', true ) ) ) ? ryse_global_var( 'scrollbar_color', 'color', true ) : '';
	$data_nicescroll_cursorwidth = ( ! empty( ryse_global_var( 'scrollbar_width', 'width', true ) ) ) ? ryse_global_var( 'scrollbar_width', 'width', true ) : '';
?>


<?php if ( ( 'default' != get_post_meta( get_the_id(), 'selectheader', true ) ) && ( get_post_meta( get_the_id(), 'selectheader', true ) ) ) { ?>
	<body <?php body_class(); ?> data-page-transition="<?php echo esc_attr( ryse_global_var( 'page_transition_switch', '', false ) ); ?>" data-header-style="header-style-<?php echo esc_attr( get_post_meta( get_the_id(), 'selectheader', true ) ); ?>" data-nicescroll-cursorcolor="<?php echo esc_attr( $data_nicescroll_cursorcolor ); ?>" data-nicescroll-cursorwidth="<?php echo esc_attr( $data_nicescroll_cursorwidth ); ?>">
<?php } elseif ( ( ryse_global_var( 'header-style', '', false ) ) ) { ?>
	<body <?php body_class(); ?> data-page-transition="<?php echo esc_attr( ryse_global_var( 'page_transition_switch', '', false ) ); ?>" data-header-style="<?php echo esc_attr( ryse_global_var( 'header-style', '', false ) ); ?>" data-nicescroll-cursorcolor="<?php echo esc_attr( $data_nicescroll_cursorcolor ); ?>" data-nicescroll-cursorwidth="<?php echo esc_attr( $data_nicescroll_cursorwidth ); ?>">
<?php } else { ?>
	<body <?php body_class(); ?> data-page-transition="0" data-nicescroll-cursorcolor="09276f" data-nicescroll-cursorwidth="10px">
<?php } ?>
	...
	if ( class_exists( 'Radiantthemes_Addons' ) ) {
		if ( ! is_user_logged_in() && ! empty( ryse_global_var( 'coming_soon_switch', '', false ) ) ) {
			...
			die;
		} elseif ( ! is_user_logged_in() && ! empty( ryse_global_var( 'maintenance_mode_switch', '', false ) ) ) {
			include ABSPATH . 'wp-content/plugins/radiantthemes-addons/maintenance.php';
			die;
		} elseif ( ! is_user_logged_in() && ! empty( ryse_global_var( 'coming_soon_switch', '', false ) ) && ! empty( ryse_global_var( 'maintenance_mode_switch', '', false ) ) ) {
			include ABSPATH . 'wp-content/plugins/radiantthemes-addons/coming-soon.php';
			die;
		}
	}
	...
	<?php if ( ryse_global_var( 'preloader_switch', '', false ) ) { ?>
		<!-- preloader -->
		<div class="preloader" data-preloader-timeout="<?php echo esc_attr( ryse_global_var( 'preloader_timeout', '', false ) ); ?>">
    		<?php
    		if ( ! empty( ryse_global_var( 'preloader_style', '', false ) ) ) {
    			include get_parent_theme_file_path( 'inc/preloader/' . ryse_global_var( 'preloader_style', '', false ) . '.php' );
    		}
    		?>
		</div>
		<!-- preloader -->
	...
	<?php if ( ryse_global_var( 'page_transition_switch', '', false ) ) { ?>
	...
	<!-- scrollup -->
	<?php if ( ryse_global_var( 'scroll_to_top_switch', '', false ) ) { ?>
		<?php if ( ! empty( ryse_global_var( 'scroll_to_top_direction', '', false ) ) ) : ?>
			<div class="scrollup <?php echo esc_attr( ryse_global_var( 'scroll_to_top_direction', '', false ) ); ?>">
		<?php else : ?>
	...
	<?php if ( ryse_global_var( 'gdpr_notice_switch', '', false ) ) { ?>
		<!-- gdpr-notice -->
		<div class="gdpr-notice alert alert-dismissible">
			...
			<div class="row">
				<div class="col-lg-9 col-md-9 col-sm-8 col-xs-12 text-left">
					<p><?php echo esc_html( ryse_global_var( 'gdpr_notice_content', '', false ) ); ?></p>
				</div>
				<div class="col-lg-3 col-md-3 col-sm-4 col-xs-12 text-center">
					<a class="btn" href="<?php echo esc_url( ryse_global_var( 'gdpr_notice_button_link', '', false ) ); ?>"><?php echo esc_html( ryse_global_var( 'gdpr_notice_button_text', '', false ) ); ?></a>
				</div>
			</div>
			<!-- row -->
		</div>
	...
	<?php if ( class_exists( 'ReduxFrameworkPlugin' ) ) { ?>
		<?php if ( 'full-width' === ryse_global_var( 'layout_type', '', false ) ) { ?>
			<div class="radiantthemes-website-layout full-width">
		<?php } elseif ( 'boxed' === ryse_global_var( 'layout_type', '', false ) ) { ?>
	...
	<?php } ?>
		...
		if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
			...
			if ( is_404() || is_search() ) {
				if ( ( ryse_global_var( 'header-style', '', false ) ) ) {
					include get_parent_theme_file_path( 'inc/header/' . ryse_global_var( 'header-style', '', false ) . '.php' );
				} else {
					include get_parent_theme_file_path( 'inc/header/header-style-default.php' );
				}
			} else {
				...
				} elseif ( ( ryse_global_var( 'header-style', '', false ) ) ) {
					include get_parent_theme_file_path( 'inc/header/' . ryse_global_var( 'header-style', '', false ) . '.php' );
				} else {
					include get_parent_theme_file_path( 'inc/header/header-style-default.php' );
				}
		...
		<!-- hamburger-menu-holder -->
			...
			<?php
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				if ( ( 'header-style-default' != ryse_global_var( 'header-style', '', false ) ) ) {
					if (
