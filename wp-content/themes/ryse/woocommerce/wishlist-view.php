...
?>
			...
			<th class="product-name">
				<span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'ryse' ) ); ?></span>
			</th>
			...
			<?php if ( $show_price ) : ?>
				...
				<th class="product-price">
					<span class="nobr">
						<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'ryse' ) ); ?>
					</span>
				</th>
...
?>
			...
			<?php if ( $show_stock_status ) : ?>
				...
				<th class="product-stock-status">
					<span class="nobr">
						<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'ryse' ) ); ?>
					</span>
				</th>
...
?>
		...
		if ( count( $wishlist_items ) > 0 ) :
				...
				if ( $product && $product->exists() ) :
					...
					<tr id="yith-wcwl-row-<?php echo wp_kses_post( $item['prod_id'] ); ?>" data-row-id="<?php echo wp_kses_post( $item['prod_id'] ); ?>">
						...
						<td class="product-remove">
							<div>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ); ?>" class="remove remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'ryse' ) ); ?>"><span class="ti-close"></span></a>
							</div>
						</td>
						<?php endif; ?>
						...
						<?php if ( $show_price ) : ?>
							<td class="product-price">
								...
								echo wp_kses_post( $base_product ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'ryse' ), $product ) );
								?>
							</td>
						<?php endif ?>
						...
						<?php if ( $show_stock_status ) : ?>
							<td class="product-stock-status">
								<?php echo wp_kses_post( $stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'ryse' ) . '</span>' : '<span class="wishlist-in-stock">' . __( 'In Stock', 'ryse' ) . '</span>' ); ?>
							</td>
						<?php endif ?>
						...
						<td class="product-add-to-cart">
							...
							if ( $show_dateadded && isset( $item['dateadded'] ) ) :
								echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'ryse' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
							endif;
							?>
							...
							<select class="change-wishlist selectBox">
								<option value=""><?php esc_html_e( 'Move', 'ryse' ); ?></option>
								<?php
								foreach ( $users_wishlists as $wl ) :
									...
									<option value="<?php echo esc_attr( $wl['wishlist_token'] ); ?>">
										...
										if ( $wl['wishlist_privacy'] == 1 ) {
											$wl_privacy = __( 'Shared', 'ryse' );
										} elseif ( $wl['wishlist_privacy'] == 2 ) {
											$wl_privacy = __( 'Private', 'ryse' );
										} else {
											$wl_privacy = __( 'Public', 'ryse' );
										}
							...
							<?php if ( $is_user_owner && $repeat_remove_button ) : ?>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ); ?>" class="remove_from_wishlist button" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'ryse' ) ); ?>"><?php _e( 'Remove', 'ryse' ); ?></a>
							<?php endif; ?>
						</td>
					<?php endif; ?>
					</tr>
			...
			?>
			<tr>
				<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products were added to the wishlist', 'ryse' ) ); ?></td>
			</tr>
			<?php
		endif;
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo wp_kses_post( $ask_estimate_url ); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if ( ! is_user_logged_in() ) : ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', __( 'Your email', 'ryse' ) ); ?></label>
			...
			<button class="btn button ask-an-estimate-button ask-an-estimate-button-popup" >
				...
				<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'ryse' ) ); ?>
			</button>
		</form>
	</div>
<?php endif; ?>
