...
<div class="wrap about-wrap rt-admin-wrap">
	...
	<h2 class="nav-tab-wrapper wp-clearfix">
		<a class="nav-tab" href="<?php echo esc_url( self_admin_url( 'themes.php?page=radiantthemes-dashboard' ) ); ?>"><?php esc_html_e( 'Dashboard', 'ryse' ); ?></a>
		<a class="nav-tab nav-tab-active" href="<?php echo esc_url( self_admin_url( 'themes.php?page=radiantthemes-admin-plugins' ) ); ?>"><?php esc_html_e( 'Install Plugins', 'ryse' ); ?></a>
			<?php if ( defined( 'FW' ) ) { ?>
		...
		<div class="welcome-content w-clearfix extra">
			<div class="radiantthemes-plugins radiantthemes-theme-browser-wrap">
				<div class="theme-browser rendered">
					<div class="whi-install-plugins-wrap">
						<h3><?php echo esc_html__( 'These below plugins are required', 'ryse' )?></h3>
						<a href="#" class="radiantthemes-admin-btn whi-install-plugins"><?php echo esc_html__( 'Activate all plugins', 'ryse' ) ?></a>
					</div>
					...
					foreach( $plugins as $plugin ) :
						...
						if ( strpos( $plugin_action, 'deactivate' ) !== false ) {
							...
							$plugin_action = '<div class="row-actions visible active"><span class="activate"><a class="button radiantthemes-admin-btn">' . esc_html__( 'Activated', 'ryse' ) . '</a></span></div>';
						}
			...
			<?php if ( defined( 'FW' ) && ! get_option('fw:ext:backups:log') ) : ?>
				<div class="radiantthemes-notice radiantthemes-notice-success plugin-install-success">
					<p><a href="<?php echo esc_url( self_admin_url( 'tools.php?page=fw-backups-demo-content' ) ); ?>"><?php echo esc_html__( 'Click Here to continue with Demo Import process.', 'ryse' ); ?></a></p>
				</div>
			<?php endif; ?>
		</div>
	</div>
